...
class NewRowSynthesis(SingleTableMetric):
    ...
    def compute_breakdown(
        cls, real_data, synthetic_data, metadata=None, numerical_match_tolerance=0.01, synthetic_sample_size=None
    ):
        """Compute this metric.
        ...
        for index, row in synthetic_data.iterrows():
            ...
            for field in real_data.columns:
                ...
                if pd.isna(row[field]):
                    ...
                    field_filter = f"abs(`{field}` - {row[field]}) <= " f"{abs(numerical_match_tolerance * row[field])}"
